...
#' @author mg14
#' @export
PlotVarianceComponents <- function(fit, col=1:nlevels(fit$groups), groups = fit$groups, type="rowSums", conf.int=FALSE, absolute=TRUE, digits=2, var=c("var2","var"), order=TRUE,trans,CIs=NULL,...) {
	var <- match.arg(var)
	...
	pie(vp, col=col[1:length(vp)], border=NA, labels= paste(sapply(strsplit(names(vp),"_"),function(x) x[[1]]),CIs), radius = 0.8, init.angle=90,...)
	
	if(length(vn)>0){
		par(new=T)
		pie(c(abs(vn), sum(vp)-sum(vn)), col=c(col[names(vn)],NA), border=NA, labels=paste(names(vn), " (", round(vn, digits),rn,")", sep=""), new=FALSE, density=c(rep(36, length(vn) ),NA), radius = 0.8, init.angle=90,...)
	}
...
WaldTest <- function(coxRFX, var=c("var2","var"), uncentered=TRUE){
	...
	if(uncentered){
		...
		names(v[!names(v)%in%names(c)]) <- paste0("mu.",names(coxRFX$mu[coxRFX$mu!=0]))
